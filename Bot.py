...
def get_stockData(ticker):
    ...
    stockData.dif = roundby2(get_change(last_quote,get_data_with_fallback(info,"previousClose",0)))
    stockData.high = prettify(roundby2(get_data_with_fallback(info,"fiftyTwoWeekHigh",0)))
    stockData.low = prettify(roundby2(get_data_with_fallback(info, "fiftyTwoWeekLow", 0)))
    stockData.cap = millify(get_data_with_fallback(info ,'marketCap', 0), precision=2, drop_nulls=False)
    stockData.profits = millify(get_data_with_fallback(info,"grossProfits", '0'), precision=2, drop_nulls=False)
    stockData.currency = get_data_with_fallback(info,'currency','')
